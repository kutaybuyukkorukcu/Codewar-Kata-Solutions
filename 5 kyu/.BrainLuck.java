...
public class BrainLuck {
	...
	public String process(String input) {
		int iPointer = 0, memoryPointer = 0;
		String result = "";
		for (int i = 0; i < code.length(); i++) {
			char c = code.charAt(i);
			if (c == '>') memoryPointer++;
			else if (c == '<') memoryPointer--;
			else if (c == '+') saveToMemory(memoryPointer, (char) (getFromMemory(memoryPointer) + 1));
			else if (c == '-') saveToMemory(memoryPointer, (char)(getFromMemory(memoryPointer) - 1));
			else if (c == '.') result += getFromMemory(memoryPointer);
			else if (c == ',') {
				if (getFromMemory(memoryPointer) == 0) {
					saveToMemory(memoryPointer, input.charAt(iPointer));
					iPointer++;
				}
	...
	private int getMatching(char x, char match, int i, int dir) {
		...
		while (i > 0 && i <= code.length()) {
			...
			else if (code.charAt(i) == match) count--;
			if (count == 0) break;
			i += dir;
		}
		return i;
